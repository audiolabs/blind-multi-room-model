...
class DecayDataset(Dataset):
    ...
    def __init__(
        ...
        if not testset_flag:
            ...
            # Load EDCs
            f_edcs = h5py.File(
                datasets_dir + "synthEDCs/edcs_100{}.mat".format(n_slopes_str), "r"
            )
            edcs = np.array(f_edcs.get("edcs"))

            # Load noise values
            f_noise_levels = h5py.File(
                datasets_dir + "synthEDCs/noiseLevels_100{}.mat".format(n_slopes_str),
                "r",
            )
            ...
            assert not torch.any(
                torch.isnan(edcs_db_normalized)
            ), "NaN values in normalized EDCs"
            assert not torch.any(
                torch.isinf(edcs_db_normalized)
            ), "Inf values in normalized EDCs"
            ...
            assert (
                self.edcs.shape[1] == self.noise_levels.shape[1]
            ), "More EDCs than noise_levels"
            ...
            if not exactly_n_slopes_mode:
                ...
                for slope_idx in range(1, n_slopes_max + 1):
                    self.n_slopes[
                        0, (slope_idx - 1) * edcs_per_slope : slope_idx * edcs_per_slope
                    ] = (slope_idx - 1)
                self.n_slopes = self.n_slopes.long()
        else:
            f_edcs = h5py.File(datasets_dir + "motus/edcs_100.mat", "r")
            edcs = (
                torch.from_numpy(np.array(f_edcs.get("summer830edcs/edcs")))
                .float()
                .view(-1, 100)
                .T
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.testset_flag:
            ...
            if self.exactly_n_slopes_mode:
                n_slopes = torch.empty(
                    0
                )  # just return empty, because n_slopes is already fixed
            else:
...
class DecayFitNet(nn.Module):
    def __init__(
        ...
        self.linears = nn.ModuleList(
            [
                nn.Linear(
                    round(n_max_units * (reduction_per_layer**i)),
                    round(n_max_units * (reduction_per_layer ** (i + 1))),
                )
        ...
        # T_vals
        self.final1_t = nn.Linear(
            round(n_max_units * (reduction_per_layer ** (n_layers - 1))), 50
        )
        self.final2_t = nn.Linear(50, n_slopes)

        # A_vals
        self.final1_a = nn.Linear(
            round(n_max_units * (reduction_per_layer ** (n_layers - 1))), 50
        )
        self.final2_a = nn.Linear(50, n_slopes)

        # Noise
        self.final1_n = nn.Linear(
            round(n_max_units * (reduction_per_layer ** (n_layers - 1))), 50
        )
        ...
        self.exactly_n_slopes_mode = exactly_n_slopes_mode
        if not exactly_n_slopes_mode:
            self.final1_n_slopes = nn.Linear(
                round(n_max_units * (reduction_per_layer ** (n_layers - 1))), 50
            )
    ...
    def forward(self, edcs):
        ...
        if not self.exactly_n_slopes_mode:
            ...
            n_slopes = torch.empty(
                0
            )  # just return None, because n_slopes is already fixed
...
def edc_loss(
    ...
    t = (torch.linspace(0, l_edc * fs - 1, round((1 / 0.95) * l_edc * fs)) / fs).to(
        device
    )
    ...
    edc_prediction = generate_synthetic_edc_torch(
        t_vals_prediction, a_vals_prediction, n_vals_prediction, t, device
    )
...
class FilterByOctaves(nn.Module):
    ...
    def __init__(
        self, center_frequencies=None, order=5, sample_rate=48000, backend="scipy"
    ):
    ...
    def _get_octave_filters(
        center_freqs: List, fs: int, order: int = 5
    ) -> List[torch.Tensor]:
        """
...
def check_format(rir):
    ...
    if rir.shape[0] > rir.shape[1]:
        ...
        print(
            f"Swapped axes to bring rir into the format [{rir.shape[0]} x {rir.shape[1]}]. This should coincide "
...
def rir_onset(rir):
    spectrogram_trans = torchaudio.transforms.Spectrogram(
        n_fft=64, win_length=64, hop_length=4
    )
...
class PreprocessRIR(nn.Module):
    ...
    def __init__(
        ...
        self.filterbank = FilterByOctaves(
            order=5,
    ...
    def forward(
        self, input_rir: torch.Tensor, input_is_edc: bool = False, analyse_full_rir=True
        ...
        if input_is_edc:
            ...
            schroeder_decays, norm_vals = self.schroeder(
                input_rir, analyse_full_rir=analyse_full_rir
            )
        ...
        if self.output_size is not None:
            schroeder_decays_db = torch.nn.functional.interpolate(
                schroeder_decays_db,
        ...
        if self.input_transform is not None:
            schroeder_decays_db = (
                2 * schroeder_decays_db / self.input_transform["edcs_db_normfactor"]
            )
        ...
        time_axis = torch.linspace(
            0,
            (schroeder_decays.shape[2] - 1) / self.sample_rate,
            schroeder_decays_db.shape[2],
        )

        # Reshape freq bands as batch size, shape = [batch * freqs, timesteps]
        schroeder_decays_db = schroeder_decays_db.view(
            -1, schroeder_decays_db.shape[-1]
        ).type(torch.float32)

        return schroeder_decays_db, time_axis, norm_vals, scale_adjust_factors

    def schroeder(
        self, rir: torch.Tensor, analyse_full_rir=True
...
def _postprocess_parameters(
    t_vals, a_vals, n_vals, scale_adjust_factors, exactly_n_slopes_mode
):
...
def decay_model(
    t_vals, a_vals, n_val, time_axis, compensate_uli=True, backend="np", device="cpu"
):
    ...
    assert np.all(also_zero_a), (
        "T values equal zero detected, for which A values are nonzero. This "
        "yields division by zero. For inactive slopes, set A to zero."
    )
    t_vals[t_vals == 0] = 1

    if backend == "np":
        edc_model = generate_synthetic_edc_np(
            t_vals, a_vals, n_val, time_axis, compensate_uli=compensate_uli
        )
        return edc_model
    elif backend == "torch":
        edc_model = generate_synthetic_edc_torch(
            t_vals,
            a_vals,
            n_val,
            time_axis,
...
def generate_synthetic_edc_torch(
    t_vals, a_vals, noise_level, time_axis, device="cpu", compensate_uli=True
) -> torch.Tensor:
    ...
    if compensate_uli:
        exp_offset = (
            exponentials[:, :, -1].unsqueeze(2).repeat(1, 1, time_axis.shape[0])
        )
    else:
        exp_offset = 0
...
def generate_synthetic_edc_np(
    t_vals, a_vals, noise_level, time_axis, compensate_uli=True
) -> np.ndarray:
...
def schroeder_to_envelope(schroeder_T, schroeder_A, fs):
    assert (
        schroeder_T.shape == schroeder_A.shape
    ), "Dimensions mismatch between T and A."
    assert np.any(
        schroeder_T.shape == schroeder_A.shape
    ), "modelT and modelA have different sizes."
