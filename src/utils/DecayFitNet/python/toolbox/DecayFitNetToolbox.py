...
class DecayFitNetToolbox:
    def __init__(
        ...
        self._session = onnxruntime.InferenceSession(
            os.path.join(path_onnx, model_name)
        )

        # Load input transform
        self._input_transform = pickle.load(
            open(os.path.join(path_onnx, f"input_transform{n_slope_str}.pkl"), "rb")
        )

        self._preprocess = PreprocessRIR(
    ...
    def preprocess(
        self, signal: torch.Tensor, analyse_full_rir=True
    ...
    def estimate_parameters(
        self, input: torch.Tensor, input_is_edc: bool = False, analyse_full_rir=True
        ...
        """
        edcs, __, norm_vals, scale_adjust_factors = self._preprocess(
            input, input_is_edc, analyse_full_rir
        )

        ort_inputs = {
            self._session.get_inputs()[0].name: DecayFitNetToolbox._to_numpy(edcs)
        }
        ...
        t_prediction, a_prediction, n_prediction = _postprocess_parameters(
            t_prediction,
            a_prediction,
            n_prediction,
